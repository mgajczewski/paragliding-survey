# Survey Categories

This document visualizes the categories of "benefits" and "concerns" associated with each proposed solution in the Ballast & Fairness survey.

## Categories per Solution (Mermaid Diagram)

```mermaid
mindmap
  root((Survey Categories))
    Solution 1: Equalizers
      Concerns
        Safety/Certification
        Liability/Certification
        Safety/Installation
        Unfairness
        Unfairness (performance gap)
        Safety (light pilots ballasting)
        Market (incentives for small wings)
        Market Stagnation
        Technological Stagnation
        Philosophy (adding drag)
        Gaming the system
        Logistics
      Benefits
        Levels the field
        Improves Safety
        Promotes small wings
        Promotes micro sizes
        Promotes intermediate sizes
        Keeps "Overall" ranking
        Fairness
        Simple to Implement
    Solution 2: Partial Weight Class
      Concerns
        Dilutes "Overall" Title
        Safety (incomplete solution)
        Insufficient Incentive
        Arbitrary Cutoff
        Community Fragmentation
        Diluted Competition
      Benefits
        Improves Safety
        Improves Inclusivity
        Promotes Small Wings
        Additional Option
        Simple to Implement
        Simple to Verify
        Preserves "Overall" Winner
        Preserves Ranking
    Solution 3: Comprehensive Weight Classes
      Concerns
        Loses "Overall" Winner
        Community Fragmentation
        Diluted Competition
        Devalues Titles
        Logistics
        Team Scoring
        Ranking Fragmentation
        Enforcement
      Benefits
        Improves Safety
        Maximizes Fairness
        Promotes Innovation
        Promotes Small Wings
        Proven Concept
    Solution 4: Strict Ballast Limitations
      Concerns
        Enforcement
        Unfair
        Unfairness
        Complicated
      Benefits
        Simple Rule
        Improves Safety
        Partial Incentive
    Solution 5: MRT
      Concerns
        Randomness
        Safety
        Too Complex
        Wrong Focus
        Safety (doesn't solve ballast issue)
        Task Setter Dependency
      Benefits
        Improves Safety
        Keeps "Overall" ranking
        No physical modification
    Solution 6: Hybrid System
      Concerns
        Dilutes Competition
        Community Fragmentation
        Too Complicated
        Devalues "Champion" Title
        Resource Intensive
      Benefits
        Pilot Choice
        Solves the Problem for Those Who Want It
        Evolutionary Approach
        Preserves Tradition
```

## Proposed Higher-Level Categories

Based on the detailed points above, we can start grouping them into more generic themes. This can help in analyzing the overall sentiment and priorities of the community.

Here is a first draft of such higher-level categories:

*   **Safety**
    *   *Description*: Concerns or benefits related to pilot safety, equipment certification, and incident liability.
    *   *Examples*: `Improves Safety`, `Safety/Certification`, `Safety/Installation`, `Safety (light pilots ballasting)`, `Safety (incomplete solution)`, `Safety (doesn't solve ballast issue)`.

*   **Fairness & Equality**
    *   *Description*: How a solution impacts the level playing field for pilots of different weights.
    *   *Examples*: `Levels the field`, `Maximizes Fairness`, `Unfairness`, `Randomness`, `Unfairness (performance gap)`, `Unfairness (doesn't solve disparity)`, `Fairness (addresses performance gap)`.

*   **Competition Prestige & Tradition**
    *   *Description*: How a solution affects the value, structure, and tradition of competition titles and rankings.
    *   *Examples*: `Keeps "Overall" ranking`, `Dilutes "Overall" Title`, `Devalues Titles`, `Preserves Tradition`.

*   **Implementation & Logistics**
    *   *Description*: The practical aspects of introducing and managing a solution. This includes complexity, enforcement, and resource requirements.
    *   *Examples*: `Simple to Implement`, `Logistics`, `Enforcement`, `Too Complex`, `Resource Intensive`.

*   **Market & Technical Innovation**
    *   *Description*: The effect of a solution on glider development, market incentives for manufacturers, and overall technological progress.
    *   *Examples*: `Promotes small wings`, `Market Stagnation`, `Technological Stagnation`, `Promotes Innovation`.

*   **Community & Pilot Experience**
    *   *Description*: The impact on the pilot community, social dynamics, and the overall experience of racing.
    *   *Examples*: `Community Fragmentation`, `Pilot Choice`, `Wrong Focus`.
